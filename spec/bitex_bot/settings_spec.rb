  ...
  describe '#to_hash' do
    it 'returns a symbolized hash' do
      described_class.to_hash.should eq(
        ...
        time_to_live: 20,
        buying: { amount_to_spend_per_order: 10, profit: 0.5, fx_rate: 1 },
        selling: { quantity_to_sell_per_order: 0.1, profit: 0.5, fx_rate: 1 },
    ...
    context 'taker' do
      {
        ...
        let(:taker_hash) { { market => market_settings } }
        let(:taker_class) { "BitexBot::Api::#{market.capitalize}".constantize }

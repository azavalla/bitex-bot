...
module BitexBot
  ...
  class Robot
    ...
    def stop_opening_flows?
      msg = 'Not placing new orders, %<currency> target not met'
      (log(:debug, msg % Settings.quote) if alert?(:maker, :fiat, :stop)) ||
        (log(:debug, msg % Settings.base) if alert?(:maker, :crypto, :stop)) ||
        (log(:debug, msg % :USD) if alert?(:taker, :fiat, :stop)) ||
        (log(:debug, msg % Settings.quote) if alert?(:taker, :crypto, :stop))
    end

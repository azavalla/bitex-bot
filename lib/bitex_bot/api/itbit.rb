  ...
  module Api
    ...
    class Itbit < Wrapper
      ...
      def place_order(type, price, quantity)
        ...
        Robot.log(:error, 'Captured Timeout on itbit')
        latest = last_order_by(price)
        return latest if latest.present?

        Robot.log(:error, 'Could not find my order')
        raise e
      end
      ...
      def last_order_by(price)
        orders.select { |o| o.price == price && (o.created_time - Time.now.to_i).abs < 500 }.first
      end

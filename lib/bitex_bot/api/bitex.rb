  ...
  module Api
    ...
    class Bitex < Wrapper
      ...
      def send_order(type, price, quantity)
        { sell: ::Bitex::Ask, buy: ::Bitex::Bid }[type].create!(Settings.maker.order_book, quantity, price)
      end
      ...
      def user_transaction_parser(trade)
        UserTransaction.new(
          trade.id, trade.amount, trade.quantity, Settings.fx_rate, trade.fee, trade_type(trade), trade.created_at.to_i
        )
      end

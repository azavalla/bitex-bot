module BitexBot
  module Api
    class Kraken < Wrapper
      # Wrapper for kraken client responses.
      class Order
        cattr_accessor :last_closed_order, :wrapper
        ...
        def self.create!(type, price, quantity)
          ...
          order_id = order_info_by(type, price.truncate(1), quantity.truncate(8))['txid'].first
          find(order_id)
        ...
        def self.order_info_by(type, price, quantity)
          private_request.add_order(pair: 'XBTUSD', type: type, ordertype: 'limit', price: price, volume: quantity)
        end

        def self.find(id)
          new(*private_request.query_orders(txid: id).first)
        ...
        def self.open
          private_request.open_orders['open'].map { |o| new(*o) }
        rescue KrakenClient::ErrorResponse
          retry
        end
        ...
        #   closed: {
        #     'O75MLD-64OIU-5O4JDM': {
        #       refid: nil, userref: nil, status: 'canceled', reason: 'User canceled', opentm: 1440289877.8097,
        #       closetm: 1440291917.7096, starttm: 0, expiretm: 0, descr: {
        #         pair: 'ETHEUR', type: 'buy', ordertype: 'limit', price: 1.19980, price2: '0', leverage: nil,
        #         order: 'buy 1100.00000000 ETHEUR @ limit 1.19980'
        #       },
        #       vol: '1100.00000000', vol_exec: '0.00000000', cost: '0.00000', fee: '0.00000', price: '0.00000', misc: '',
        #       oflags: 'fciq'
        #     },
        #     ...
        #   }
        ...
        def self.closed(start: 1.hour.ago.to_i)
          private_request.closed_orders(start: start)[:closed].map { |o| new(*o) }
        rescue KrakenClient::ErrorResponse
          retry
        end
        ...
        def cancel!
          private_request.cancel_order(txid: id)
        rescue KrakenClient::ErrorResponse => e

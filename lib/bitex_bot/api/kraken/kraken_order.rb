  ...
  module Api
    ...
    class KrakenOrder
      ...
      def self.order_info_by(type, price, quantity)
        wrapper.client.private.add_order(pair: 'XBTUSD', type: type, ordertype: 'limit', price: price, volume: quantity)
      end

      def self.find(id)
        new(*wrapper.client.private.query_orders(txid: id).first)
      ...
      def self.open
        wrapper.client.private.open_orders['open'].map { |o| new(*o) }
      rescue KrakenClient::ErrorResponse
        retry
      end
      ...
      def self.closed(start: 1.hour.ago.to_i)
        wrapper.client.private.closed_orders(start: start)[:closed].map { |o| new(*o) }
      rescue KrakenClient::ErrorResponse
        retry
      end
      ...
      def cancel!
        wrapper.client.private.cancel_order(txid: id)
      rescue KrakenClient::ErrorResponse => e
    ...
    class OrderArgumentError < StandardError
    end
  end
end
